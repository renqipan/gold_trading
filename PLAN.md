...
5. XGBoost 使用 30 个 `regime_explainable` 特征预测候选交易 `P(profit first)`。输入覆盖中长期趋势、波动、美元/风险市场和 GLD 资金流；HMM 概率因与策略状态重复且跨重训块可能漂移，不再进入 XGBoost。
...
- XGBoost 策略闸门：验证段 XGBoost 过滤策略相对趋势/CUSUM/ATR fallback 的收益增益 >= 0.2 个百分点、Sharpe 增益 >= 0，且至少产生 8 次真实交易动作。
- 买入阈值：从预先给定的有限阈值网格中仅用验证段选择。当前 7.5% 候选阈值有 38 个模型信号、覆盖率约 69%，但验证期只有 5 次真实交易动作，证据不足，因此正式策略禁用 XGBoost 入场过滤。
...
- 正式次日开盘、盘中障碍历史测试毛收益：205.08%。
- 同期买入持有收益：125.13%。
- 5bps 成本后策略净收益：201.51%。
- 标准日收益 Sharpe：2.06；5bps 后 2.04；Sortino 3.68。
- 最大回撤：-10.88%，同期买入持有最大回撤为 -25.73%。
- 测试期交易动作：26 次，仓位受 12% 计划单笔止损风险上限和 100% 总仓位上限约束。
- 8bps 严格执行模拟收益：199.40%，标准日收益 Sharpe 2.03，最大回撤 -10.88%。
- XGBoost `regime_explainable` 30 特征模型的验证段 raw AUC 为 0.79，测试段 AUC 为 0.70。若强制使用验证选出的 7.5% 阈值，测试段 5bps 净收益由 fallback 的 201.51% 降至 175.65%，即贡献 -25.86 个百分点；Sharpe 从 2.04 升至 2.26，说明模型倾向于牺牲趋势收益换取更平滑路径。正式策略以绝对收益目标为先，且验证交易证据不足，因此禁用 XGBoost 交易接管。

正式策略选择风险约束后的 120 日趋势 + CUSUM + ATR，并用 walk-forward HMM 负责退出；XGBoost 仅输出研究评分。严格执行口径下，5bps 净收益高于同期买入持有，同时最大回撤明显更低。2026-06-20 之后 forward holdout 只有 15 个交易日、期间没有新交易，因此仍需继续积累真正前瞻样本，不能将本次历史提升理解为未来收益保证。
