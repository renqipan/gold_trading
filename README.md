...
最新交易日尚未结束时，页面会分别标注“行情交易日”和完整的北京时间模型生成时间，避免把 COMEX 交易日与程序运行日期混为一谈。当天价格会参与最新趋势、HMM 状态、仓位建议和回测快照；前瞻账本仍只在交易日结束后纳入该日，避免把不断变化的盘中收益固化为历史记录。已发布的前瞻账本快照保持不变；黄金日线或 FRED 利率被数据供应商事后修订时，网站历史与回测可采用修订值，但不会重写已经发布的前瞻账本。策略版本、操作动作、持有/空仓指引和待执行信号仍接受严格一致性校验。
...
由于 GitHub 的计划任务可能明显延迟或偶尔丢失，工作流会在每个目标时段后 `30` 分钟和
`90` 分钟各增加一次恢复唤醒。轻量时段检查只允许每个六小时时段执行一次完整更新，因此网站
仍然每天最多发布四次；若行情和模型没有变化，也会发布新的北京时间网站更新时间。后到达的
重复任务会在安装依赖前直接跳过。恢复唤醒只补偿尚未发布的目标时段，不会把目标时间之前的
行情伪装成该时段的网站更新；发布脚本也会拒绝提交早于目标时段的更新时间。
...
提交四个 `public/data/*.json`；本地手动脚本在行情和模型语义均无变化时仍不创建空提交，
GitHub Actions 则会为每个有效时段提交最新的网站更新时间。
